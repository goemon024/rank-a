...
import { QuestionWithUserAndTags, AnswerWithUser, CommentWithUser } from '@/types'
    ...
    useEffect(() => {
        const fetchQuestion = async () => {
            try {
                ...
                const { answers: answerList, count } = await answersRes.json() as { answers: AnswerWithUser[], count: number }
                setAnswers(answerList)
                setCount(count)

                const commentsRes = await fetch(`/api/questions/${questionId}/comments`)
                const { comments: commentList } = await commentsRes.json() as { comments: CommentWithUser[] }
    ...
    return (
        <div>
            ...
            <div className={styles.container}>
                ...
                {comments.filter((comment) => !comment.answerId).map((comment) => (
                ...
                {answers.map((answer) => (
                    <div key={answer.id}>
                        ...
                        {comments.filter((comment) => comment.answerId === answer.id).map((comment) => (
                            <CommentCard key={comment.id} comment={comment} />
                        ))}
                    </div>
                ))}
            </div>
            {isAnswerModalOpen && <AnswerModal setOpen={setIsAnswerModalOpen} questionId={questionId} />}
            {isCommentModalOpen && <CommentModal setOpen={setIsCommentModalOpen} questionId={questionId} answerId={selectedAnswerId?.toString()} />}
        </div>
    )
}   