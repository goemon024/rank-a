...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!userIdParam || !questionIdsParam) {
            return NextResponse.json({ error: "userIdとquestionIdsは必須です" }, { status: 400 });
        }

        const userId = parseInt(userIdParam, 10);
        if (isNaN(userId)) {
            return NextResponse.json({ error: "ログインしていない、userIdが不正です" }, { status: 400 });
        }
        ...
        if (questionIds.length === 0) {
            return NextResponse.json({ error: "ブックマークがない、或いは、questionIdsが不正です" }, { status: 400 });
        }
