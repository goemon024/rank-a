...
export async function PATCH(req: NextRequest,) {
    ...
    const questionId = parseInt(req.nextUrl.pathname.split("/").filter(Boolean)[2], 10);
    ...
    try {
        ...
        const bestAnswerId = body.bestAnswerId === null ? null : parseInt(body.bestAnswerId, 10);

        if (bestAnswerId !== null && typeof bestAnswerId !== "number") {
            return NextResponse.json({ error: "Invalid bestAnswerId" }, { status: 400 });
        }
