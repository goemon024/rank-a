...
export async function DELETE(req: NextRequest) {
    ...
    try {
        ...
        if (existingVote.userId !== userId) {
            return NextResponse.json({ error: "Not authorized to delete this vote" }, { status: 403 });
        }
...
export async function PUT(req: NextRequest) {
    ...
    try {
        ...
        if (existingVote.userId !== userId) {
            return NextResponse.json({ error: "Not authorized to delete this vote" }, { status: 400 });
        } //403

        let newtype: "Upvote" | "Downvote";
        const { type: typeFromRequest } = await req.json();
        if (typeFromRequest === "Upvote") { newtype = "Upvote"; }
        else if (typeFromRequest === "Downvote") { newtype = "Downvote"; }
        else { return NextResponse.json({ error: "Invalid vote type" }, { status: 400 }); }

        console.log("change vote type to", newtype);
        await prisma.vote.update({ where: { id: voteId }, data: { type: newtype } });


        return NextResponse.json({ message: "Vote changed successfully" }, { status: 200 });
    } catch (err) {
