...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error("DELETE /voteQuestion/[voteId] エラー:", err);
      return NextResponse.json({ error: "Invalid token" }, { status: 401 });
    }

    console.log("DELETE voteId", voteId);

    const existingVote = await prisma.voteQuestion.findUnique({
      where: { id: voteId },
    });
    ...
    await prisma.voteQuestion.delete({ where: { id: voteId } });
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error("PUT /voteQuestion/[voteId] エラー:", err);
      return NextResponse.json({ error: "Invalid token" }, { status: 401 });
    }
